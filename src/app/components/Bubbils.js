...
export function Bubbles({
  ...
  const bubbleSpeed = useRef(Array.from({ length: count }));
  ...
  useFrame(() => {
    ...
    material.color = new THREE.Color("#cbcbcb");
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (o.position.y > 4 && repeat) {
        o.position.y = -10; // Reset to bottom
        o.position.x = gsap.utils.random(-20, 20);
        o.position.z = gsap.utils.random(-20, 20);
      }
